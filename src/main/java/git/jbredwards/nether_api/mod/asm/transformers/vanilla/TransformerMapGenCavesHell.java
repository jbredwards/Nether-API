 ...
 *
 */
public final class TransformerMapGenCavesHell implements ITransformer
{
    @Nonnull
    @Override
    public byte[] transform(@Nonnull String name, @Nonnull String transformedName, @Nonnull byte[] basicClass) {
        return transform(basicClass, true, classNode -> {
            ...
            @Nonnull final IntegerHolder biome = new IntegerHolder();
            transformMethod(classNode, method -> method.name.equals(DEOBFUSCATED ? "addTunnel" : "func_180704_a"), (method, insn) -> {
                if(insn == method.instructions.getFirst()) {
                    biome.value = method.localVariables.stream().mapToInt(lv -> lv.index).max().orElseThrow(IllegalStateException::new) + 1;
                    ...
                    method.localVariables.add(new LocalVariableNode("biome", "Lnet/minecraft/world/biome/Biome;", null, start, end, biome.value));
                }
                /*
                 ...
                 */
                    ...
                    if(((FieldInsnNode)insn).name.equals(DEOBFUSCATED ? "FLOWING_LAVA" : "field_150356_k")) {
                        ((JumpInsnNode)insn.getNext()).setOpcode(IFNE);
                        method.instructions.remove(insn.getPrevious());
                        method.instructions.insertBefore(insn, new MethodInsnNode(INVOKEINTERFACE, "net/minecraft/block/state/IBlockState", DEOBFUSCATED ? "getMaterial" : "func_185904_a", "()Lnet/minecraft/block/material/Material;", true));
                        method.instructions.insertBefore(insn, new MethodInsnNode(INVOKEVIRTUAL, "net/minecraft/block/material/Material", DEOBFUSCATED ? "isLiquid" : "func_76224_d", "()Z", false));
                        method.instructions.remove(insn);
                    }
                    // iblockstate.getBlock() instanceof IFluidBlock
                    else if(((FieldInsnNode)insn).name.equals(DEOBFUSCATED ? "LAVA" : "field_150353_l")) {
                 ...
                 */
                else if(insn.getOpcode() == DSTORE && ((VarInsnNode)insn).var == (DEOBFUSCATED ? 57 : 56)) {
                    final InsnList list = new InsnList();
                    list.add(new VarInsnNode(ALOAD, 0));
                    list.add(new FieldInsnNode(GETFIELD, "net/minecraft/world/gen/MapGenBase", DEOBFUSCATED ? "world" : "field_75039_c", "Lnet/minecraft/world/World;"));
                    list.add(new VarInsnNode(ILOAD, 3));
                    list.add(new VarInsnNode(ILOAD, 4));
                    list.add(new VarInsnNode(ILOAD, 50));
                    list.add(new VarInsnNode(ILOAD, 53));
                    list.add(genHookMethod("getBiome", "(Lnet/minecraft/world/World;IIII)Lnet/minecraft/world/biome/Biome;"));
                    list.add(new VarInsnNode(ASTORE, biome.value));
            ...
            // recursiveGenerate
            transformMethod(classNode, method -> method.name.equals(DEOBFUSCATED ? "recursiveGenerate" : "func_180701_a"), (method, insn) -> {
                /*
